...
# ==== 3. 推理和保存 ====
def batch_predict_softlabel_dataloader(ecg_loader, ecg_model, tqn_model, label_features, device="cuda"):
    soft_labels = []
    with torch.no_grad():
        for batch in tqdm(ecg_loader, desc='Inference'):
            ...
            out = tqn_model(feats, label_features)  # [B, num_labels, 2]
            if out.shape[-1] == 2:
                soft_prob = torch.softmax(out, dim=-1)[..., 1]  # [B, num_labels]
            else:
                soft_prob = torch.sigmoid(out)
            soft_labels.append(soft_prob.cpu().numpy())
...
if __name__ == "__main__":
    # ========== 路径配置 ==========
    ckpt_path="/data_C/sdb1/lyi/ked3/control-spiderman-ECGFM-KED-456810e/trained_model/checkpoints_mimiciv/best_valid_all_increase_zhipuai_augment_epoch_7.pt"
    ...
    label_csv = "/data_C/sdb1/lyi/ked/ECGFM-KED-main/dataset/all_label_standard.csv"   # 包含标签文本
    label_column = "label"
    # 数据文件夹，所有 update_remain_part_x.csv 都在这里
    data_dir = "/data_C/sdb1/lyi/ecg-chat/ECG-Chat-master/edge_1/dataset/data/split/"
    batch_size = 128
    tqn_layers = 7
    ...
    # 获取所有 update_remain_part_*.csv 文件
    # file_list = [f for f in os.listdir(data_dir) if f.startswith("update_remain_part_") and f.endswith(".csv")]
    # file_list.sort()  # 按顺序处理
    ...
    for csv_file in file_list:
        ...
        soft_labels = batch_predict_softlabel_dataloader(
            ecg_loader, ecg_model, tqn_model, label_features, device
        )

        save_softlabel_path = os.path.join(data_dir, f"ecg_softlabel_save_{csv_file.replace('.csv','.npy')}")
        np.save(save_softlabel_path, soft_labels)
        print(f"{csv_file} 推理完成，软标签保存于: {save_softlabel_path}")